...
namespace bumperbot_planning {
    AStarPlanner:: AStarPlanner(): Node("a_star_node") { // constructor
       ...
       // ---------------------------------------------------- ROS 2 INTERFACE ------------------------------------------------------------------
        ...
        map_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("/a_star/visited_map", map_qos); // need to match the QoS durability policy with map_sub_
    }
    ...
    // ------------------------------------------------------ FUNCTIONS, DEFINED ----------------------------------------------------------------
        ...
        nav_msgs::msg::Path AStarPlanner::plan(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal) {
            ...
            while(!pending_nodes.empty() && rclcpp::ok()) {
                ...
                visited_map_.data.at(poseToCell(active_node)) = -106; // this is an occupancy grid, want to mark neighours of node as visited and to also visualize in RViz
                                                                    // -106 has no real meaning. Just offers a nice visualisation. orange colour...?
